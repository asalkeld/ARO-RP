...
func TestDeployARMTemplate(t *testing.T) {
	...
	for _, tt := range []struct {
		name    string
		mocks   func(*mockfeatures.MockDeploymentsClient)
		wantErr error
	}{
		{
			name: "Deployment successful with no errors",
			mocks: func(dc *mockfeatures.MockDeploymentsClient) {
				dc.EXPECT().
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			deploymentsClient := mockfeatures.NewMockDeploymentsClient(controller)
